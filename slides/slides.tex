...
\subtitle{Bilinear Resize + Map-based Rectify on Zynq UltraScale+}
...
targeting the ZCU104 Zynq UltraScale+ board.  Both are verified
...
% =====================================================================
\begin{frame}[fragile]{Resize Kernel: bilinear, 2-row ring buffer, II=1}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.52\textwidth}
...
\begin{frame}{Rectify Kernel: \code{m\_axi} baseline + optimisation roadmap}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.43\textwidth}
      \textbf{Measured performance (1920$\times$1080):}\\[0.3em]
      \begin{tabular}{ll}
        \toprule
        cycles      & 8{,}294{,}489 \\
        wall @200MHz & $\approx$ \hi{41 ms} ($\approx$ 24 fps) \\
        \bottomrule
      \end{tabular}
      \vspace{0.6em}
...
bottlenecked by memory dependencies, which works out to about 41
milliseconds for a full HD frame -- enough for 24 fps but not 60.  The
...
% =====================================================================
\begin{frame}{Results: all green, 1--3\,\% of \code{xczu7ev}}
  \textbf{Verification (target \code{xczu7ev-ffvc1156-2-e} @ 5\,ns):}\\[0.3em]
  \centering
  \begin{tabular}{lcccc}
    \toprule
    kernel  & py-tests & C-sim & csynth slack & co-sim \\
    \midrule
    resize  & 5/5 & 5/5 & $-0.00$\,ns (marginal) & \hi{10/10} (5 synth + 5 NYU)\\
  ...
  \begin{tabular}{lcccccc}
    \toprule
    kernel  & BRAM & DSP & FF & LUT & \% of part & throughput \\
    \midrule
    resize  & 2  & 37 & 5{,}206 & 6{,}406 & \hi{1--2\,\%} & 1 px / cycle \\
    rectify & 12 & 13 & 7{,}542 & 8{,}903 & \hi{1--3\,\%} & 4 cyc / px   \\
    \bottomrule
  \end{tabular}\\[0.7em]

  \raggedright
  \textbf{Takeaway:} A streaming line-buffer schedule turns resize into a
  near-free PL block; rectify is correct \& small but memory-bound
  until the buffered path lands.  Golden-first methodology transferred
  cleanly from one kernel to the next.
\end{frame}
\note{%
Final numbers: every layer of verification passes for both kernels.
Resize's top-module slack is reported as minus-zero nanoseconds at the
5\,ns target -- that's an HLS sub-rounding flag, not a functional
failure; cosim is clean.  Rectify meets the same 5\,ns target with no
warning.  Resource-wise, both kernels sit under 3\% of the
\code{xczu7ev} on every metric, which leaves plenty of headroom for
the rest of a perception pipeline.  The takeaway: the line-buffer
streaming schedule makes resize essentially free in the PL, rectify is
correct and small but memory-bound until we land the buffered variant,
and the golden-first methodology transferred from one kernel to the
next without modification.  Happy to take questions.%
}
